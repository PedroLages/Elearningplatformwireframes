...
- **Commit before review**: Third story in a row where review found uncommitted working-tree fixes. Always commit all changes before running `/review-story` — uncommitted code creates false blockers that waste a review cycle.
- **Extract shared utilities early**: `highlightMatches()` and `truncateSnippet()` started inline in the component. Code review flagged them as untestable. Extracting to `searchUtils.ts` with dedicated tests caught edge cases (empty query, overlapping matches) that would have slipped through component-level tests alone.
- **Test coverage for ACs with metrics**: Acceptance criteria with specific metrics (e.g., "within 100ms", "highlighted matching keywords") need explicit test assertions, not just behavioral smoke tests. The testing review caught two ACs with zero targeted coverage.
